...
from config import constants, model_config
from config.constants import *
from collections import deque
#import tensor flow and related items
...
class Agent:
    ...
    def __init__(self, state_size, window_size, trend, skip):
        ...
        self.model = Model(self.state_size, model_config.DQN_OUTPUT_SIZE, model_config.DQN_LAYER_SIZE, model_config.DQN_LEARNING_RATE,
                           'real_model')
        self.model_negative = Model(self.state_size, model_config.DQN_OUTPUT_SIZE, model_config.DQN_LAYER_SIZE, model_config.DQN_LEARNING_RATE,
                                    'negative_model')
    ...
    def _memorize(self, state, action, reward, new_state, dead, rnn_state):
        ...
        if len(self.MEMORIES) > model_config.DQN_MEMORY_SIZE:
            self.MEMORIES.popleft()

    def _select_action(self, state):
        if np.random.rand() < model_config.DQN_EPSILON:
            action = np.random.randint(model_config.DQN_OUTPUT_SIZE)
        else:
    ...
    def _construct_memories(self, replay):
        ...
        Y = np.empty((replay_size, model_config.DQN_OUTPUT_SIZE))
        INIT_VAL = np.empty((replay_size, 2 * model_config.DQN_LAYER_SIZE))
    ...
    def buy(self, initial_money, load_model):
        if load_model:
            ...
            saver.restore(self.sess, "./model/dqn_model.pt")
        ...
        init_value = np.zeros((1, 2 * model_config.DQN_LAYER_SIZE))
    ...
    def train(self, iterations, checkpoint, initial_money):
        for i in range(iterations):
            ...
            for t in range(0, len(self.trend) - 1, self.skip):
                ...
                self.INITIAL_FEATURES = new_state
                batch_size = min(len(self.MEMORIES), model_config.DQN_BATCH_SIZE)
                ...
                model_config.DQN_T_COPY += 1
                model_config.DQN_EPSILON = model_config.DQN_MIN_EPSILON + (1.0 - model_config.DQN_MIN_EPSILON) * np.exp(-model_config.DQN_DECAY_RATE * i)
            if (i + 1) % checkpoint == 0:
                print('epoch: %d, total rewards: %f.3, cost: %f, total money: %f' % (i + 1, total_profit, cost,
                                                                                     starting_money))
        saver = tf.train.Saver()
        save_path = saver.save(self.sess, "./model/dqn_model.pt")
...
class DQNAgent(BaseSignalAgent):
    ...
    def signal(self):
        self.lock.acquire()
        df = self.broker_agent.ohlcv_data(SYMBOL,TIMEFRAME, limit=1000)
        close = df[constants.PRICE_COL].values.tolist()
        window_size = 30
        skip = 1
        ...
        df['Sell_Signal'] = np.where(last_action[0] ==-1, 1, 0)
        df['Sell_Position'] = df['Sell_Signal'].diff()
        df['Buy_Signal'] = np.where(last_action[0] == 1, 1, 0)
